...
declare var $: any;
declare var musicData: object;

var activeMediaIndex = 3
var cacheUserPlaylistData = {}
var activeLoading = false
sessionStorage.removeItem('activeView')

export function hideCurrentView(id: string) {
  if (id) {
...
async function openAlbum(id: string, library: Boolean) {
  if (activeLoading) {
  ...
  if (library) {
    var openAlbumLibraryStatus = 'lib'
    var showFullAbum = `<button data-toggle="tooltip" data-placement="top" title="Show Full Album" onclick="openAlbum('${id}')" class="animated fadeInUp btn-text-primary"> <i class='bx bx-list-plus'></i> </button>`
  }
  else {
    var openAlbumLibraryStatus = ''
    var showFullAbum = ''
  }
  ...
  if (!library) {
    ...
    var data = await goFetch(`albums/${id}`)

    var musicData[id] = data
    var openAlbumData = {
      id: data.id,
