...
async function performSearch(val) {
  ...
  const result = await fetch(
    `https://api.spotify.com/v1/search?q=${val}&type=album,artist,playlist,track`,
    {
      method: "GET",
      headers: {
...
async function buildSearch(data) {
  ...
  for (let i = 0; i < data.tracks.items.length; i++) {
    ...
    try {
      ...
      d.setAttribute(
        "track_details",
        id + "//==//" + data.tracks.items[i].external_urls.spotify
      );
        ...
        <div class="row">
            <div class="col-4 playlist_full_bar1">
                <center>
                    ...
                    <br><br>
                    <button onclick='playAlbum(${JSON.stringify(
                      data.tracks.items
