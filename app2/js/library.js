...
async function loadLibrarySpotify() {
  ...
  if (!doc.exists) {
    ...
    for (let i = 0; i < doc.data().albums.length; i++) {
      ...
      i.innerHTML = `
        <img onclick="loadAlbum('${album.id}')" src="${album.art}">
        <h4>${album.name}</h4>
        <p>${album.artists}</p>
      `
...
async function loadLibraryPlaylists() {
  ...
  for (let i = 0; i < query.docs.length; i++) {
    ...
    e.classList.add("playlist");
    e.classList.add("playlist_user");
    ...
    for (let k = 0; k < playlist.songs.length; k++) {
      ...
      trackLengthSnippet = `~${Number.parseFloat(
        track.length / 60000
      ).toPrecision(2)}s`;
    ...
    f.innerHTML = `
        <div class="${classes}">
            <button onclick="hidePlaylist('${
              query.docs[i].id
            }')" class="btn-text-primary iconbtn exitbtn">
            ...
            <div class="row">
                <div class="col-4 playlist_full_bar1">
                    <center>
                        ...
                        <img id="${
                          query.docs[i].id
                        }cover" class="animated fadeIn" src="${playlist.cover}">
                        <br><br>
                        <h2 class="animated fadeInUp">${playlist.name}</h2>
                        <p class="animated fadeIn">${playlist.owner.name}</p>
                        <br><br>
                        <div class="playlistActions">
                            <button onclick='playPlaylist(${JSON.stringify(
                              playlist.songs
                            )})' class="btn-contained-primary">play</button>
                            <div class="dropdown">
                                <button aria-haspopup="true" class="btn-text-primary iconbtn" data-toggle="dropdown" aria-expanded="false">
                                    <i class="material-icons">more_horiz</i>
                                </button>
                                <div class="dropdown-menu menu">
                                    <a onclick="setDescription('${
                                      query.docs[i].id
                                    }', '${efilter(
      playlist.description
    )}', this)" class="dr-item">Set Description</a>
                                    <a onclick="setCover('${
                                      query.docs[i].id
                                    }')" class="dr-item">Set Playlist Cover</a>
                                    <div class="dropdown-divider"></div>
                                    <a onclick="deletePlaylist('${
                                      query.docs[i].id
                                    }')" class="dropdown-item waves-effect btn-danger">Delete Playlist</a>
                                </div>
                            </div>
                        </div>
                        <hr>
                        <br>
                        <p id="${query.docs[i].id}desc">${
      playlist.description
    }</p>
                    </center>
                </div>
                ...
                <ul id="${
                  query.docs[i].id
                }playlistView" class="list-group musicList">
                    ${songSnippet}
                </ul>
...
async function add_to_playlist(playlist) {
  ...
  trackLengthSnippet = `~${Number.parseFloat(song.length / 60000).toPrecision(
    2
  )}s`;
...
async function confirmCover(id) {
  ...
  window.setTimeout(() => {
    ...
    document.getElementById(id + "cover").src =
      ...
      "?alt=media&" +
      new Date().getTime();
    document.getElementById(id + "cover2").src =
      "https://firebasestorage.googleapis.com/v0/b/eonsound.appspot.com/o/covers%2F" +
      id +
      "." +
      ext +
...
async function deletePlaylist(id) {
  x = confirm(
    "Are you sure you want to delete this playlist? This action is irreversible.\n\n\n\n\n\n"
  );
  if (!x) {
